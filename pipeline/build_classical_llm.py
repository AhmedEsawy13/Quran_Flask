...
def build_messages(book, surah, name, prose, verses, chunk_idx=0, n_chunks=1):
    ...
    system = (
        ...
        '- "reported_from": اسم العالِم فقط إذا كان الحكم (grade) في هذا الصفّ '
        'بعينه هو رأي ذلك العالِم المنقول، لا رأي المؤلف نفسه — وإلا null.\n'
        'تنبيه مهم على هذا الحقل تحديدًا: كثيرًا ما يذكر المؤلف حكمه الخاصّ أوّلًا '
        'ثم يذكر أنّ عالِمًا آخر خالفه برأيٍ مختلف، مثل: «كاف، وقال نافع: تام». '
        'هنا حكم المؤلف نفسه هو «كاف» فـ reported_from لهذا الصفّ = null؛ '
        'ورأي نافع «تام» إن استخرجته كصفٍّ مستقلٍّ بنفس الموضع فـ reported_from '
        'لذلك الصفّ المستقلّ فقط = «نافع». لا تضع اسم عالِمٍ على صفٍّ حكمُه '
        'ليس هو ما قاله ذلك العالِم.\n\n'
